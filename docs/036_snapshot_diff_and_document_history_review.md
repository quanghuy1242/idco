...
- [x] **J2 — Productionize the `ReviewModel`.** (Shipped 2026-07-02; see [`docs/038`](038_woven-overlay-design.md) §5.4.) The ghost dispatch moved from the react-view top-level loop into `EngineBlock` behind a `ReviewRenderContext`, so the SAME ghost-aware path runs at the top level AND inside a container's child assembly; the `ReviewModel` became a recursive projection (`view/review-model.ts`: merged top-level `order`, `ghosts` at all depths, per-container merged `childOrder`, per-container `collapsed` budget); `seedFor` seeds a ghost from its base node's metrics (content-aware, no pop-in) and the id→height cache is evicted on review exit; the ghost box moved margin→padding so it is fully measured; the **per-container ghost budget** bounds a deletion-heavy container (containers do not internally virtualize). Two grounded corrections to the plan: (a) the **top-level region threshold is not a cost necessity** — the treap virtualizes top-level ghosts, so only the per-container budget is load-bearing; the threshold-to-diff-view is a UX affordance that rides the review affordance (J4); (b) the **scoped-`DiffView` drill-in and the visible "+N collapsed" affordance are J3** (the T3 drill-in and the passive-layer DOM surfacing) — J2 bounds cost and records `collapsed` in the model. Incremental idle-coalesced re-diff rides J6 (editing-during-review); the hard no-tear gate rides J6 (it tests the focused-block-protection handshake J6 adds). Tests: `engine-review-model.test.ts` + `review-ghost-spike.spec.ts` (J2.5 in-container ghost, J2.6 budget) + screenshots.
- [x] **J3 — Passive marker layer + element ring + color.** (Shipped 2026-07-02; see [`docs/038`](038_woven-overlay-design.md) §5.5.) Generalized the R6-I change indicator to the passive marker layer at ANY depth: a new pure `changedElements(diff)` router walks the diff's merged spine recursively and routes each change by WHERE it sits and WHAT it is — a top-level non-unchanged block keeps the R6-I left-inset gutter `bar` (position-safe, and hue-safe outside the prose box), and a NESTED element with a direct element-level change (`.attrs` or `.object` — a re-colored table cell, an object whose field changed) gets an on-content two-tone element `ring`. `applyReviewIndicators` sets/clears `data-engine-review-ring` alongside the bar; `useReviewChangeIndicator` decorates at any depth while still returning the top-level list for the host's count. The **color system** (038 §9): status is carried by SHAPE (bar vs ring), the bar keeps a status hue in the safe inset, and the ring is a two-tone box-shadow so its edge survives on any content color. Two grounded corrections from two adversarial passes: (a) the ring must use BOTH an `outline` AND an inset `box-shadow`, because the two ring-target classes have opposite constraints — a nested text block's inline `outline:none` (`blockStyle`) kills an outline-only ring, and a nested object's hover/live `box-shadow` chrome (`ENGINE_OBJECT_CHROME_CSS`) replaces a box-shadow-only ring; the two are independent, composing properties, so at least one channel always paints; (b) an inset (not outset) box-shadow, so a `<td>` ring is not occluded by the adjacent cell in a `border-collapse` table. Deferred with owners: T1 woven run-text track-changes (wash/strike on inserted/deleted runs) → J6 (needs the optimistic-apply live-proposed/ghost distinction); the T2 floating detail chip + the T3 scoped-`DiffView` drill-in + the visible "+N collapsed" affordance → J4 (need the review cursor / single overlay-authority surface); a top-level object takes the bar, not §8's on-element ring — that ring+drill-in IS the T3/J4 affordance. Tests: `engine-review-decoration.test.tsx` (the pure any-depth router + a two-channel-ring CSS-invariant guard + the DOM apply/clear) + `review-decoration.spec.ts` (bar + three rings incl. a nested code-block object, the object ring surviving the hover chrome, the router leaving table/callout as bars, deletion-tick composition) + legibility screenshots.
- [x] **J4 — Active review surface + review cursor.** (Shipped 2026-07-02; see [`docs/038`](038_woven-overlay-design.md) §5.6.) The single active surface (§7) plus the review cursor: `useReviewCursor(diff, {onReveal})` (headless — the diff's changed top-level blocks as ordered stops, next/prev/goTo with wrap, each landing reveals via `scrollToBlock`, index reconciled across a re-diff so a resolved change advances sanely) with its pure `reviewCursorEntries`/`reviewEntryDetail`; and `ReviewCursorSurface`, exactly one anchored control at a time on the cursor's block, carrying the change detail + prev/next + accept/reject (block + whole) + a view-diff drill-in, `taking`-focus with a `focusEditor` reclaim on a terminal action. Whole accept/reject is functional over the pure `applyProposal` (J1) in the story; the editor package stays policy-free (the surface emits scoped intents, never mutates the store). Grounded corrections to the plan: (a) the L3 surface is a **standalone consumer-wired anchored control** (the shipped `comment-affordance.tsx` mould), NOT an overlay-authority contributor — the authority is internal editor chrome unreachable by the opt-in review layer, every other review piece (the R6-I indicator, J2/J3's model + markers) is consumer-wired, and "exactly one" is true BY CONSTRUCTION with a single cursor, so the authority's one-winner arbitration earns nothing here (its coexistence/co-slot value returns only when in-review text editing raises the flyout beside the surface — J6, not J4); (b) a **removed** change has no live element, so it reveals + anchors via the surviving neighbor ABOVE the deletion gap (the R6-I deletion-anchor concept, preference reversed vs the gutter tick so the ghost stays on screen), with the all-removed-document edge deferring to a review-order-aware reveal seam; (c) the accept/reject **affordance** is J4, but the review-mode-correct **resolution plumbing** (optimistic `origin:"suggested"` apply, review-local undo, save gate, focused-block handshake) is J6 — so per-block accept's moving-baseline and a rebuild-on-reject host's focus reclaim both ride J6. Adversarially reviewed to settled (3 rounds: removed-change reveal + ghost-on-screen geometry, reject-focus/J6 coupling, flyout-coexistence-is-convention). Tests: `engine-review-cursor.test.tsx` (cursor nav/wrap/reconcile, `revealId` neighbor logic, detail summaries) + `review-cursor.spec.ts` (one surface, nav scrolls, a removal reveals its ghost IN-VIEWPORT, focus reclaim on accept, functional accept-keeps/reject-reverts) + legibility screenshots.
- [x] **J5 — SuggestionSource SPI + Changes pane.** (Shipped 2026-07-02; see [`docs/038`](038_woven-overlay-design.md) §5.7.) The host-owned proposal registry + the pane that carries the anchorless remainder: a `SuggestionSource` SPI (`view/spi/suggestion-source-registry.ts`, the exact `comment-source-registry.ts` sibling — module singleton, register-by-id, idempotent, registration-order listing, `active` = first) with `load`/`create`/`accept`/`reject`/`update`/`subscribe`; a **Changes** dock pane (`view/chrome/panes/changes-pane.tsx`, registered in `command-builtins` gated on `activeSuggestionSource()`, mirroring Comments) that computes each proposal's review from the live snapshot (`applyProposal` → `diffSnapshots` → `reviewCursorEntries` for the block-anchored jumps) and routes the **anchorless remainder** through a new pure `anchorlessChanges(diff, conflicts)` (`core/suggestions/changes-routing.ts`): conflicts, settings, and collection changes — the three classes with no `[data-engine-block-id]` to weave onto (038 §17). Grounded decisions: (a) the SPI drops §7.3's `docId` param to match the shipped comment sibling's host-closure-per-document model, and makes `subscribe` first-class (an async agent produces a proposal *after* the pane loaded); (b) the pane is a **read-only preview** (docs/038 §12) — it never mutates the store, so accept/reject only record the host outcome and move the proposal to Resolved; the optimistic in-store apply / moving-baseline / save-gate are J6; (c) settings/collections read from the diff (they always apply, never conflict) while conflicts read from `applyProposal(...).conflicts`, so a change is either woven, anchorless, or a conflict — never double-counted; (d) the per-proposal apply+diff is keyed on `current.revision` (via the commit-only `useReviewSnapshot`), not the snapshot object, so a stepless caret-move commit does not re-diff every proposal on navigation. Adversarially reviewed to settled (no load-bearing defect; the one substantiated finding — a navigation re-diff from a selection-subscribed snapshot — fixed). Tests: `engine-suggestion-source.test.ts` (registry idempotency/order/active; the `anchorlessChanges` router incl. no-op-collection filtering, field-count summary, ordering; an end-to-end real-engine conflict + collection route) + `review-changes.spec.ts` (the pane lists a host proposal, routes the conflict + glossary change, resolves on accept) + legibility screenshots.
- [x] **J6 — Review-mode plumbing (load-bearing correctness).** (Shipped 2026-07-02; see [`docs/038`](038_woven-overlay-design.md) §5.8.) Enter/exit review mode now lives on `EditorStore`; human edits during review record into a separate arbiter-exempt `HistoryPool` and get `origin:"suggested"` while programmatic optimistic apply/revert stays `recordHistory:false` and non-interactive; focus reclaim keys on the dispatch entry point via `CommittedTransaction.interactive`, not `origin`; `applyProposalToStore` performs the focused-block-protection handshake before removal/reparenting ops, collects whole-proposal and per-block inverses, and `revertReviewEdits` replays the review segment before reject; public saves are gated through `canSaveSnapshot`/`OwnedEditorHandle.canSave`; autosave waits while the gate is closed; block resolution cuts the review segment with `markReviewResolutionBoundary`. Story: `stories/engine-review-mode.stories.tsx`. Tests: `engine-review-mode.test.ts`.
- [x] **J7 — Attribution wiring.** (Shipped 2026-07-02; see [`docs/038`](038_woven-overlay-design.md) §5.9.) Under single-proposal review the author is constant; `proposalAttribution` maps the proposal author to a stable chip label and hue, `attributionForTextRun` resolves identity-backed changed `TextRunDiff` runs to that author, and `ReviewCursorSurface` shows the optional author chip beside the status. Story: `stories/engine-review-mode.stories.tsx`. Tests: `engine-suggestion-attribution.test.ts`.
- [x] **J8 — Public API map + docs.** (Shipped 2026-07-02.) `pnpm build` regenerated the package API maps; `@quanghuy1242/idco-editor` reports 362 exports across 26 categories with summaries/categories at 100%, including the J6/J7 review-mode and attribution public symbols. Final gate: `pnpm check` green.
