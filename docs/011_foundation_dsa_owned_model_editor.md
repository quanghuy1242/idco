...
Virtualization windows over the body's top-level `children` (the body order array); each entry resolves to its subtree by id in O(1). The grain is the top-level block, and a block mounts with its whole subtree — so whether mounting is cheap depends on whether a block's subtree is bounded. Three of the node kinds keep it bounded by construction: a text leaf is one paragraph; an object sequesters its internals in opaque `data` (§2.7); and a **list is not a subtree at all** — list items are flat top-level `listitem` leaves carrying a depth attribute (the structural `list` container is never produced by editing or import; see 010 §10.4 / 018 §2.10), so an N-item list is N top-level blocks the window covers one item at a time, never one block that mounts whole. The only kind whose subtree can be unbounded is a **genuine multi-block structural container** (a quote/callout holding block children), and that is exactly where the windowing contract below recurses.

**The windowing contract, and who implements it.** Virtualization is one contract: *report your full height, mount only your viewport, and let the model own selection across the offscreen remainder.* Selection composes through it for free, because §8.5 already paints only mounted leaves and lets the model hold the rest. The contract is universal; the only variable is **who implements it**, and the rule is: **whoever can enumerate the internals owns the windowing.**

- A **structural node** exposes `children` that are first-class nodes the engine can enumerate, so the **engine implements the contract generically** — the same `calculateVirtualRange` applied one level down, composing offsets as `container offset + child offset within the container's window`. No per-node code; recursive virtualization is a property of the structural kind itself.
- An **object node** is opaque `data` the engine cannot enumerate (§2.7), so the **object's definition implements the contract itself**: it declares its full/estimated height to the outer window math (the mounted DOM is only its viewport slice, so a measured `offsetHeight` would be wrong) and opts into nested scroll when it is taller than the viewport.

Same contract, different implementor, chosen by enumeration. This is why object-internal virtualization is a node responsibility and structural-container virtualization is an engine responsibility — not two policies, one contract with a delegation rule. Because structural nodes are a first-class extension point (§2.3), recursive structural windowing is a **framework capability, not an application's deferred edge**: an extension author who builds a large container gets windowing for free. Object authors get the contract plus a reusable windowed-list primitive (the same `calculateVirtualRange` exposed as a utility) so delegation is batteries-included, not a per-node reimplementation — and a small object ignores all of it and just renders. The live-book grid (§2.4) is the first real object that implements its half; the recursive structural tier is built when an extension produces a large container, against the seam this contract names.
