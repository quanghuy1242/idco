...
- **Status: hardening pass (post-landing).** A round of polish/bug fixes on the landed surface, all opt-in, standard editor untouched, full suite green:
  - **Selection overlay repaints on commit, not only selection-dirty.** A mark/link/block-type toggle keeps the model selection identical (`tr.setSelection(store.selection)`), so `dispatch` reported `selectionChanged === false` and the selection lane never fired — the painted rects stayed pinned to the pre-toggle (bare-text) geometry while the leaf re-rendered into mark spans (a ~1s stale-selection lag). `view/store-hooks.ts` `SelectionFrameStore` now also schedules its frame task on `subscribeCommit` (coalesced, so ≤1 extra geometry pass per commit).
  - **"Double selection" over formatted runs.** `Range.getClientRects()` across the nested mark elements (`<strong>`/`<em>`/`<a>`) returns a rect for each contained *element box* and its *text*, so the semi-transparent overlay rects stacked into a darker band over formatted runs only. `view/geometry.ts` `textRangeClientRects` now collapses rects to one per visual line (`mergeRectsByLine`), the clean highlight a native selection draws (also benefits the IME preedit underline).
  - **List bullet overlapped the text.** `blockStyle`'s inline `padding` shorthand wins the cascade over the stylesheet `padding-left`, so the `::before` bullet landed on the glyphs. `view/styles.ts` `blockStyleFor` applies the list gutter (and the indent margin) inline, after the shorthand.
  - **Indent/outdent were no-ops on flat list items.** The Payload import flattens lists to body-level `listitem` leaves (no `list` container, §14), so `currentListItem` returned null and `compileIndent` did nothing. `core/commands.ts` adds an `attrs.indent` fallback (indent any block up to a cap, outdent a flush list item back to a paragraph), rendered as a left margin; `canIndent`/`canOutdent` updated; tests in `engine-commands.test.ts`.
  - **Resting objects were boxed in editor chrome.** A permanent border+padding (`objectBlockStyle`) wrapped every baked object, including `<hr>`/embeds. Removed; an object renders bare like the reader, with only a non-layout-affecting hover/active `box-shadow` ring (`ENGINE_OBJECT_CHROME_CSS`), preserving the AC3 no-shift contract.
  - **Object popover double-flicker.** `EngineObjectBlock` used an inline `ref` callback (new identity each render → React calls it null-then-element on every re-render), nulling the popover `triggerRef` exactly as it mounted resting→live. Replaced with a stable `useCallback` ref.
  - **Click-to-edit link popover (legacy floating-link-editor parity).** `view/link-popover.tsx`: a click on an inert link mark resolves the mark by `data-engine-mark-id`, selects its range on the model, and opens an `@idco/ui` `AnchoredPopover` with Open/Apply/Remove driving the existing `set-link`/`clear-link` commands.
  - **Find is a real `@idco/ui` popover, not an inline bar (or a hand-rolled card).** It first pushed the editing surface down when it opened; it was then a floating `<div>` — but that meant it was the one editor surface not using the design-system popover (no entrance/exit animation, no shared focus/dismiss). `view/find-bar.tsx` now renders an `AnchoredPopover` against a zero-size top-right anchor, so it gets the React Aria animation, focus management, and Escape-to-close for free, with no layout shift. Because find must survive clicks into the document (reading matches), `AnchoredPopover` gained a `shouldCloseOnInteractOutside` passthrough and find keeps itself open for any interaction within `[data-engine-editor]`. Every editor popover/menu (link, object config, toolbar menus, context menu, find) now goes through `@idco/ui` — none are hand-rolled. Toolbar Search button + Ctrl/Cmd+F both open it (new `Search` icon in `@idco/ui`). Tests: `engine-toolbar.spec.ts` (find opens, searches the model, stays open while navigating).
  - **Popover exit animation (object config + link).** Those two popovers *were* real `AnchoredPopover`s but were rendered conditionally (`{open ? <AnchoredPopover/> : null}` / `if (!target) return null`), so closing unmounted them before React Aria could run `data-[exiting]:animate-popover-out` (no transition-out). Both now render unconditionally and toggle `isOpen`, so React Aria plays the exit animation and *then* unmounts (the live content still mounts only while open/exiting). The rule: a popover must be kept mounted and driven by `isOpen`, never conditionally unmounted, or it cannot animate out.
  - **UI cleanup.** Retired the dead hand-rolled `objectConfig*` inline styles (the config/media panels are already `@idco/ui` `Input`/`Button`).
  - **List-item spacing.** List items sit tighter than paragraphs (the typographic convention) via `blockStyleFor` (`LIST_ITEM_PADDING_Y`/`LIST_ITEM_MIN_HEIGHT`), with the bullet `::before` `top` kept in sync.
  - **List toggle-off + padding-collapse bug + the standardized guard.** The toolbar List button is now a real toggle (`listitem` ⇄ `paragraph`, with active state). Fixing it surfaced a React inline-style **shorthand/longhand reconciliation bug**: `blockStyle` used the `padding` shorthand while `blockStyleFor` set padding *longhands* for list items, so flipping listitem→paragraph cleared the top padding to 0 (the line collapsed). Audited every other node (baked objects, custom node views, marks, the divider, the root view): the text block is the only element restyled across renders, so the class was contained. Standardized so it cannot recur: `blockStyle`/`blockStyleFor` are typed `LonghandBlockStyle` (`Omit<CSSProperties, "padding"|"margin"|"border"|"font"|"inset"|"gap">`, a compile error to add a shorthand), `tests/editor/engine-style-invariants.test.ts` is the runtime invariant, the rule is documented in docs/017 §3.6, and the quote bar's previously-dead stylesheet `padding-left` was moved into `blockStyleFor` too. Reproduced + regression-locked in `tests/e2e/engine-list.spec.ts`.
  - **Editor↔reader resting drift (unbaked objects).** The editor's at-rest view (`BakedObjectView`) baked an unbaked object on the fly for display, but the reader's `RestingDocument` honored only `node.baked` — so imported objects (compat does not bake, to keep the round-trip deep-equal, §14) rendered as their bare type name ("embed"/"media"/"divider") in the resting/reader render while the editor showed the real baked view. The two resting renderers had drifted, the precise thing §6.2 / docs/015 say must not happen. Closed at the gap, not locally: a single shared `renderRestingObject(node, registry)` (`view/resting-document.tsx`) now backs both paths (`BakedObjectView` delegates to it), so they cannot diverge again. Regression in `engine-phase8-integration.test.tsx`. (The built-in `media`/`embed` resting views are deliberately lightweight placeholders — `🖼 src` / `🔗 url` — not a real `<img>`/`<iframe>`; richer media render is a reader-layer concern, docs/018 §2.8.)
  - **Resting render is now self-sufficient (was prose-only).** The resting/reader render used real semantic elements themed *entirely* by the host's `prose`, so a host with a CSS reset (Tailwind preflight) but no typography plugin — including the editor's own Ladle harness — got stripped defaults and no replacement: unstyled headings, bulletless lists, collapsed spacing. The editor surface never had this because it ships `ENGINE_TYPOGRAPHY_CSS`; the resting render lacked the equivalent. Fixed by `ENGINE_RESTING_TYPOGRAPHY_CSS` (`view/styles.ts`), injected by `RestingDocument`: a baseline whose every rule is `:where(…)`-wrapped to **zero specificity**, so it beats the UA/preflight reset but loses to any real `prose` (which still governs when a host installs typography). Values track `ENGINE_TYPOGRAPHY_CSS` so editor and reader match. Interim until the reader package (docs/015) owns L1 styling (docs/018 §2.5). Regression: `tests/e2e/engine-resting.spec.ts`.
  - **Toolbar parity with the legacy bar.** The block-type control is a labeled dropdown (icon + current style name + chevron; `Paragraph` / `Heading 1‑4` / `Quote` / `List item`) reading `current-block-type` + the `tag` attr (accessible name "Text style"); its menu items preview their own style (Heading 1 large+bold, Quote italic, …) like legacy. Undo/Redo are `disabled` when there is nothing to undo/redo (new `EditorStore.canUndo`/`canRedo` getters). Icon-only command buttons use `square` (DaisyUI `btn-square`) and a tighter divider, so the bar reads as dense as the legacy one. Added a `Heading 4` option.
  - **Editing-surface cursor.** Text blocks set no `cursor`, so the surface showed the default arrow; `blockStyle` now sets `cursor: text` (links re-assert `pointer`). The engine still owns caret *painting* — this is only the pointer affordance.
  - **Right-click context menu (was in no engine phase; only legacy had one).** `view/context-menu.tsx` ports the *mechanism* from `legacy/plugins/context-menu-plugin.tsx` — a React Aria `MenuTrigger` whose trigger is a zero-size `fixed` anchor at the cursor, opened on `contextmenu` — but the *actions* are pure engine commands (no Lexical). It branches by model selection: a non-collapsed selection gets inline-format toggles, a caret gets block-type + list + indent, and both get the universal edit group (Cut/Copy/Paste/Select all/Delete) through the same model clipboard serialization the native Ctrl+C/X/V path uses (via the async Clipboard API on the menu-click gesture; Cut/Copy/Delete disabled without a selection). Wired on the `OwnedModelEditor` surface. Tests: `engine-chrome.test.tsx` (jsdom: right-click toggles a mark) + `engine-toolbar.spec.ts` (chromium: real right-click → menu → bold lands on the model). New `Scissors`/`ClipboardPaste` icons registered in `@idco/ui`.
  - **Right-click no longer collapses the selection.** A right press fired the block's caret-placing `mousedown` (`focusAtClick`/`onRootMouseDown`), collapsing any selection before `contextmenu` — so the menu saw a caret, not the selection (and the selection visibly vanished). Both handlers now ignore non-primary buttons, so the selection survives for the menu (the native-editor behavior).
  - **Known gap (named, not done):** the `attrs.indent` level added for the indent/outdent fallback is editor-session-only — it is not yet projected through the compat round-trip (`pickAttrs` does not carry `indent`) nor applied by `resting-document.tsx`, so it does not persist on save and the resting/reader render does not show it. Tracked for the Phase 9 block-type/object-surface parity work (docs/018 §2.8).
...
- **Items (full list + rationale in docs/018):**
  ...
  - Editor block-type/object-surface parity (docs/018 §2.8): code-block syntax highlighting + language selector via `@idco/ui` `CodeEditor`/`CodeBlock` (keeping the AC3 no-shift height contract), and making `callout` authorable + styled (it is modeled and round-trips but is not in the block-type menu nor styled on the editing surface).
