...
The Comments pane reads threads from the source (seeded by `index.comments` of `kind: "comment"` for anchors and excerpts, hydrated by `load`/`resolve` for bodies and state) and presents them as a list grouped by **Unresolved / Resolved**, matching the docs/006 §4.6 `[Comments] [Unresolved] [Resolved]` row. Each entry shows the excerpt, the author and time, the body, and replies; actions are reply, edit, resolve/reopen, delete, and jump-to-anchor. Filtering by author and a count badge for unresolved threads belong here. The pane is the management *and* the read surface for comments: a comment in the document is opened by placing the caret in it and clicking the chip that appears (§7.5/§16.8), which routes here focused on the thread — there is no inline thread popover, because threads are long.

### 7.5 The Marks Stop Being Inert

For real comments the inert span of §3.3 becomes a live highlight: a visible background on the marked range. Opening the thread is **not** a click on the range itself — a comment spans a large range, so clicking it is ordinary caret placement, not "open" (the §16 P6 refinement). Instead, a collapsed caret inside a commented range surfaces a small chip that routes to the Comments pane focused on the thread; the highlighted text stays freely editable. (Glossary, being word-sized, keeps click-to-read; see §16.8.) A future enrichment is reflecting `resolved` in the render — a resolved thread's highlight dimming or hiding while keeping the anchor — which would need the SWR snapshot/thread state to flow into `renderAnnotationMark` (today it receives only the mark); the pane already reflects resolved/unresolved, so this is an optional polish, not a blocker.
...
> Status: the editor (live) half is implemented — `useAnnotationInteraction` + `AnnotationPopover` (glossary click-to-read), `CommentAffordance` (the comment caret chip), `panelHost.open(paneId, focusId)`, and the per-pane reveal+ring. The reader (resting) half is docs/015 §12 and is built with the reader extraction.

The marks are visible after P3/P4 (the glossary `<abbr>`, the comment highlight) but still *passive* — you cannot read a marked range or jump to its term/thread. This phase makes the annotations interactive on the live tier and is the natural payoff of the dock: a marked word becomes the entry point to its term/thread. It was surfaced during the first run-through (the marks felt inert next to the dock) and is sequenced after P5 because it depends on P3+P4 existing and reuses seams already shipped.

The interaction model, settled — and it is *not* uniform, because the trigger must match the mark's footprint:

- **Glossary → click-to-read.** A glossary mark is *word-sized*, so clicking it is a deliberate "read this," like a dictionary tooltip, and it does not fight editing. A click opens a small read popover with the single-source definition (no copy) and an "Open in Glossary" action that routes to the dock focused on the term.
- **Comment → caret affordance, not click.** A comment mark spans a *large* range (a sentence, a paragraph), so making the range a click target hijacks ordinary caret placement — the first-run-through annoyance ("I click in the paragraph and it keeps opening"). So comments are *not* a click target. Instead, when a collapsed caret lands inside a commented range, a small "Comment" chip appears anchored at that comment's highlight, and clicking it routes **straight to the dock** focused on the thread. The text stays freely clickable; because it is caret-driven it works on touch (a tap places the caret) with no hover; and threads are long, so the dock — not a popover — is the read surface. This supersedes the earlier "click-only, popover-first, both kinds" plan: that was right for glossary, wrong for comments.

This is **SPI/seam work, not bespoke wiring**:

- **Glossary click reuses the link pattern.** The surface delegates a click to `useLinkInteraction` by inspecting the target's mark element ([owned-model-editor.tsx](../packages/editor/src/view/owned-model-editor.tsx)). A sibling `useAnnotationInteraction` extends that one delegated handler to `[data-engine-glossary-term]` (the attr the mark already emits, §6.1) and opens one `AnnotationPopover` (the `LinkPopover` shape), tried before the link so the innermost mark claims the click. Delegated, virtualization-safe, no per-mark handler.
- **Comment affordance is caret-driven.** `CommentAffordance` re-evaluates on every selection/commit (`useStoreVersion`), finds a comment mark covering a collapsed caret (the marks carry `attrs.thread`, §7.5), and renders a chip anchored at that comment's highlight — the same DOM-anchoring the selection flyout uses. Collapsed-caret only, so it never overlaps the formatting flyout (which shows on a range).
- **Routing extends the dock seam.** `panelHost.open(paneId)` gained an optional focus target — `panelHost.open(paneId, focusId?)` — and each pane gained a "scroll this row/thread into view + ring it" contract via `SidePanelRenderArgs.focusId` (a shared `useScrollToFocus`). The Glossary pane rings the term row; the Comments pane rings the thread card. Small additions to the `PanelHost` and `SidePanel` SPIs, not a new mechanism.
