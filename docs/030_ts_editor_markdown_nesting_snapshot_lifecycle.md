...
> Resolution (2026-06-26): MIO-1 and MIO-2 are implemented and green under `pnpm check`. The shared correspondence is `packages/editor/src/view/markdown/transformers.ts`; export is `to-markdown.ts` (`snapshotToMarkdown`, lossy one-way, reads baked object fields, `:::tone`/`:::toc` directive grammar, GFM tables, the asserted `MARKDOWN_LOSSY_MARK_KINDS` set); import is `from-markdown.ts` (`markdownToNodes` via `markdown-it` + `markdown-it-mark`, a custom `:::` directive block rule, native marks via `boundaryAtOffset`, task lists, fenced code through the registry, code/divider/callout/toc objects, the block-bearing list item promoted to a structural `listitem`); the lossless native clipboard is `native-clipboard.ts` (`application/x-idco-snapshot`). Output is a `{ order, blocks }` fragment inserted descendant-aware by the new `compileInsertFragment` (core) over a `placeNodes` extended with a descendant resolver. The clipboard controller (`use-clipboard.ts`) reads native → markdown → HTML → plain (with the parser-free `looksLikeMarkdown` heuristic gating the plain branch) and writes native + markdown + plain on copy/cut, re-syncing focus on every path; the parser is lazy-`import()`-ed so `markdown-it` stays out of the initial bundle. **Deferred (documented, not silent):** a markdown **table on paste** is dropped with a logged note (`fragment.dropped`) — export still emits GFM tables; the **HTML-paste migration** onto the native builder (compat's last non-legacy caller) is the §7.1 follow-on and HTML paste still detours through compat. Tests: `engine-markdown-export`, `engine-markdown-paste`, `engine-markdown-clipboard` (one assertion per node/mark, security edges, round-trip, native round-trip, controller priority + focus). Post-review hardening (2026-06-26): `compileInsertFragment` remaps every node id + text-mark id to fresh store-unique ids, so a native copy→paste *into the same document* no longer collides ("Node exists"); export escapes doubled `==`/`~~` and emits a hard line break (`  \n`) so literal text and intra-paragraph newlines round-trip; object export sanitizes media/embed/post-ref hrefs with no raw fallback; the table-drop note is logged on paste. Export bakes objects *on demand* (`bakeMissingObjects`): store baking is lazy, so an object loaded from import/compat (a divider from the seed corpus) has no `baked` field — export now re-bakes via the registry (pure, deterministic) so a divider emits `---`, not `<!-- divider (unbaked) -->`; the placeholder is reserved for a genuine bake failure (media with no source). A horizontally-merged table cell (`colSpan > 1`) emits empty placeholder columns so the remaining cells stay aligned instead of shifting (the merge itself is dropped, D2). Known minor lossy edges (documented, not fixed): ordered-list `start` number (future backlog), table `rowSpan`, media caption, nested-mark seams emit valid-but-dense marker runs, and a paragraph that literally begins `:::`/`#` is not block-escaped.
...
> Resolution (2026-06-26): SN-1 (Option A) is implemented and green under `pnpm check`. Indent of a flat body-level `listitem` now promotes *only its predecessor* to a structural `listitem` holding `[prevLeaf, sublist[item]]` in place at `body.order` (`compileIndentBodyItem`, the body-root generalization of `compileIndentItem`'s else-branch); flat siblings stay windowed leaves, a second indent flows through the existing structural algebra, and outdent lifts back via `compileOutdentItem`. The reader merges a *heterogeneous* flat/structural run into one `<ul>`/`<ol>` with continuous numbering (`groupListRuns` + `renderUnit` + `structuralListItemFlavour` in `packages/reader/src/reader/render.tsx`). Markdown export emits the nested tree (`to-markdown` `structuralListItemLines`), and paste/import produce the same structural shape. A top-level structural `listitem` renders in the editor through the default stacking container (the structural-view fallback), so paste and indent-to-nest both display. The model round-trips through `toSnapshot`/`createEditorStore`. **Deferred (documented):** the promote-on-block-child trigger via *direct editing* (drag/drop a block onto an item, Enter into a block) — paste/import bring the shape in, but no live editing affordance creates it yet; a dedicated editor structural-`listitem` view with marker + gap-cursor entry (the default container renders content but not a list marker); outdent demotion of a degenerate single-leaf container back to a flat leaf; and recursive windowing of a large nested subtree (§5.3 residual cost, a native-core motivation). Tests: `engine-list-nesting` (indent promotion, flat siblings preserved, second indent, outdent, round-trip, reader heterogeneous render).
