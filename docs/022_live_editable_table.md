...
> Status (2026-06-21): substantially implemented toward legacy-Lexical-table parity on top of the docs/021 SPI. **Done:** the structural model; insert; the **live Word/Docs-style controls overlay** (`view/table-controls.tsx`, ported from the legacy `TableControlsPlugin`) — edge hover insert "+" / delete "−", column resize drag handles, the whole-table chrome with a layout selector (fixed/responsive/full-width, rescale on responsive) and a header-row / header-column / numbered-column structure menu, and remove-table; the per-cell header **bitfield** (ROW=1/COLUMN=2) matching `TableCellHeaderStates`; legacy default seed (3×3, header row+column, seeded `colWidths`); Tab/Shift-Tab cell traversal (append row past end); colWidths kept in sync on insert/delete column (delete conserves total width); and a **fidelity** compat round-trip that preserves cell `colSpan`/`rowSpan`/`backgroundColor`/`verticalAlign` and does not pad merged-cell rows. **Now at parity (2026-06-21):** rectangular cell-range selection (view-layer overlay, `view/table-interactions.tsx`; the core `EditorSelection` union is untouched, §7); cell **merge/unmerge** (`mergeCells`/`unmergeCell`, composing `set-node-attr` spans + `move-node` content + `remove-node`/`insert-node`, faithful to `@lexical/table` `$mergeCells`/`$unmergeCellNode`); column/row **move/reorder** (`moveColumn`/`moveRow` + `moveArrayItem`, refused on merged tables per `$isSimpleTable`); the right-click **context menu** (React Aria `MenuTrigger` at the cursor — insert/delete/merge/unmerge/move/delete-table); responsive reflow handled natively by the renderer (the shared `RichTextTable` emits a `%`-based `<colgroup>` from the `colWidths` ratios, so live + published tables reflow to their container via CSS — the legacy JS `ResizeObserver` was deliberately *not* ported: it was pure churn for a ratio renderer and its model write was unsound against strict-`from` undo/redo); and the document-level **geometric vertical cross-cell** caret probe (an iterative goal-column probe in `view/navigation.ts`, docs/019 §4.10 — also fixes vertical motion across mixed-width body blocks). The span-aware grid map (`tableGrid`, the `$computeTableMap` analogue), `selectedCellRange`, and the merge/move/unmerge model ops are unit-tested; the pointer/layout-driven overlay, resize, and 2D nav are typecheck/build-verified and exercised manually (jsdom has no layout). **Remaining follow-up:** grid-shaped clipboard (copy/paste/delete over a cell range — the selection and merge consume the range; the clipboard piece is future work), and insert/delete/move row/column on a *merged* table (refused via the `hasMergedCells`/`$isSimpleTable` guard and hidden in the menu so a span can never be corrupted — unmerge first, then edit; the legacy merge-aware TableObserver insert/delete path is the one feature not ported). Earlier framing: Built: the structural `table`/`tablerow`/`tablecell` model (§3); the table `StructuralDefinition` family — `createSubtree`, `fromCompatNode` (with ragged-source padding to a rectangle, §11), and the cell `toCompatNode` projection (§4.1–§4.3, the one general SPI slot added in `structural-registry.ts`); the three structural views, live + resting, replacing the read-only object stub (§4.4); insert via the generic `insert-structural`; the structure operations (insert/delete row + column, header row + column toggle) and the column-width model op `resizeColumns`, composed from the generic structural-child commands + `set-block-attr` (§6); the floating table chrome; and a fidelity-tested, byte-stable compat round-trip (incl. multi-block cells) plus an executable SPI guardrail test (§12). Cells are editable scopes (caret enters via click and arrow descent). Deferred (as this document and docs/021 §8.3 already frame them): the document-level geometric vertical cross-cell probe and smooth row-major / Tab cell traversal (§5), the resize **drag-handle UI** (the `resizeColumns` model op is built; the hover/drag chrome is not), rectangular cell-range selection (§7), and large-table windowing (§8). Where the code and this document differ, the code wins. The rest is design + rationale; no backlog or tickets here.
>
> Date: 2026-06-21
>
