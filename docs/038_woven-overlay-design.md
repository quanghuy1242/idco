...
### 5.6 J4 status — active review surface + review cursor shipped, adversarially reviewed (2026-07-02)

**J4's single active surface + the review cursor (`docs/036 §9` R6-J J4) is done.** It turns the passive map (J3) into the active review affordance (§4 L3, §7, §16): a cursor that steps through the changes and one anchored control at a time on the change under it, with accept/reject. Shipped:

- **`useReviewCursor(diff, {onReveal})`** — the headless cursor (`view/review-cursor.ts`): its stops are the diff's changed TOP-LEVEL blocks (mirrors `changedBlockIds` — the natural "next change" granularity; a nested change is reached by stepping to its top-level block and summarized in its detail, not walked cell-by-cell), a current index, `next`/`prev` (wrap) + `goTo`, each landing revealing the change via the host's `onReveal` (wired to the editor's `scrollToBlock`). The index is reconciled across a re-diff: it keeps the SAME block when it survives and advances sanely when the current change resolves and drops out (not a snap to 0). Pure `reviewCursorEntries`/`reviewEntryDetail` (a one-line summary of a `BlockDiff`) ride alongside, unit-testable and reusable by the Changes pane (J5).
- **`ReviewCursorSurface`** — the single active control (`view/overlays/review-cursor-surface.tsx`), a standalone anchored control (the shipped `comment-affordance.tsx` mould) positioned in the RIGHT GUTTER beside the cursor block (never over the change), re-measured on scroll/resize in a post-commit layout effect (the block DOM does not exist on the surface's first render). It carries the change detail + prev/next + accept/reject (this block + whole proposal) + a view-diff drill-in, all as callbacks. `taking`-focus with RECLAIM: a terminal action reclaims editor focus through `focusEditor`; the model selection survives throughout.

Grounded corrections to the plan:

- **The L3 surface is a standalone consumer-wired control, NOT the overlay authority (§4/§7 sketch).** The authority (`spi/overlay-authority.ts`) is internal editor chrome (mounted by `owned-model-editor.tsx`, driven by selection/commit) and is not reachable by the OPT-IN review layer — every shipped review piece (the R6-I indicator, J2/J3's `useReviewModel` + markers) is a consumer-wired hook/component, and the shipped single-anchored-affordance precedent is `comment-affordance.tsx` (a plain positioned control, not an authority contributor). "Exactly one active surface" is true BY CONSTRUCTION with a single cursor, so the authority's one-winner arbitration earns nothing here. Its coexistence/co-slot value (the selection flyout raised beside the surface) returns only when in-review text editing is real — J6, not J4. During the J4 read-only flow no J4 affordance ever creates a selection, so the "no flyout collision" property holds by convention (there is nothing to enforce until J6).
- **A removed change reveals + anchors via its surviving neighbor.** A removed block is absent from `store.order`, so `scrollToBlock(id)` is a no-op and the ghost (rendered in place by J2, but virtualized out when off screen) cannot be revealed by its own id. So a removed stop's `revealId` is the surviving neighbor ABOVE the deletion gap — revealing it to the viewport top leaves the ghost visible just below it (the R6-I deletion-anchor concept, preference REVERSED vs the gutter tick, because a tick has no scroll). A leading removal falls back to the following survivor (the ghost lands clipped just above the top); an all-removed document has no survivor and cannot auto-scroll — that edge rides a review-order-aware reveal seam (the invasive `scrollToBlock`-over-the-review-order change kept out of J4).
- **The accept/reject affordance is J4; the resolution plumbing is J6.** Whole accept/reject is functional today over the pure `applyProposal` (J1); the story does the baseline-moves-forward-on-accept / drop-ops-on-reject bookkeeping in CONSUMER code (the editor package is policy-free — the surface emits scoped intents and never mutates the store). Per-block ACCEPT that clears one block's marker needs the baseline to advance as blocks resolve — the optimistic-apply/moving-baseline plumbing that is J6 (§14–§16); and a host that resolves reject by rebuilding the store loses focus across that remount until J6's in-place apply removes it. So J4 ships the surface + cursor + affordance; the review-mode-correct optimistic apply, review-local undo, save gate, and focused-block handshake ride J6.

Adversarially reviewed to settled over three rounds: the removed-change reveal (a `scrollToBlock` no-op for a store-absent block → the neighbor reveal, then the ghost-on-screen geometry — prefer the neighbor ABOVE the gap), the reject-focus loss (the story's store-rebuild-on-reject, a J6-coupled limitation, documented), and the flyout-coexistence-is-convention point. Proven: `engine-review-cursor.test.tsx` (cursor nav/wrap/goTo, the same-block re-diff reconcile, `reviewEntryDetail` shapes, the `revealId` neighbor logic incl. leading/mid/trailing/all-removed) + `review-cursor.spec.ts` (exactly one surface, next/prev scroll the change into view, a REMOVAL reveals its ghost **in the viewport**, focus reclaim after accept, functional accept-keeps-and-resolves / reject-reverts-live) + screenshots in `test-results/review-cursor/`. `pnpm check` green.

### 5.7 J5 status — SuggestionSource SPI + Changes pane shipped, adversarially reviewed (2026-07-02)

**J5's Suggestion Source SPI + the Changes pane (`docs/036 §9` R6-J J5) is done.** It makes "review a proposal is the woven overlay PLUS the Changes pane" (§17) real: the host-owned proposal store, and the pane that carries the **anchorless remainder** the woven surface cannot. Shipped:

- **The `SuggestionSource` SPI** (`view/spi/suggestion-source-registry.ts`) — the exact `comment-source-registry.ts` sibling (module singleton, register-by-id, idempotent, registration-order listing, `activeSuggestionSource` = first), with `load`/`resolve?`/`create`/`accept`/`reject`/`update`/`subscribe`. The host owns storage and lifecycle; the document is never polluted with pending markup — the change is the op-log the source holds (§10, §11).
- **The pure anchorless router** `anchorlessChanges(diff, conflicts)` (`core/suggestions/changes-routing.ts`) — the load-bearing §17 split. Framework-free (reads only a `SnapshotDiff` + `applyProposal`'s `ProposalConflict[]`), it returns the flat pane list in a stable order — **conflicts** (an op whose identity anchor no longer resolves — a deleted target/text, or an `apply-failed` document-level op), then the **settings** change, then one entry per changed **collection**. These are the three classes with no `[data-engine-block-id]` to weave onto, so a reviewer would never see them without the pane. Block-anchored changes are intentionally NOT here — they weave into the document and the pane reuses J4's `reviewCursorEntries` for their jump rows.
- **The Changes dock pane** (`view/chrome/panes/changes-pane.tsx`, registered in `command-builtins` gated on `activeSuggestionSource()`, mirroring Comments) — lists the source's proposals (author, status, staleness when `baseVersion ≠ revision`), computes each proposal's review from the live snapshot (`applyProposal(current, proposal)` → `diffSnapshots` → block jumps + `anchorlessChanges`), and offers whole accept/reject.

Grounded decisions: (a) **the SPI drops §7.3's `docId`** to match the shipped comment sibling's host-closure-per-document model, and makes `subscribe` first-class (unlike `CommentSource`, which the pane polls) because an async agent produces a proposal *after* the pane loaded and the pane cannot poll it; (b) **the pane is a read-only preview** (§12), not review mode — it never mutates the store, so accept/reject only record the host outcome and move the proposal to Resolved; the optimistic in-store apply / moving-baseline / save-gate that make resolution mutate the document are J6 (§14–§16); (c) **the partition is total** — an op is either woven (block-anchored), anchorless (settings/collection, which always apply), or a conflict (a lost anchor), never double-counted, because `set-settings`/`set-collection` never conflict at the model layer while a conflict is by definition an op that could not apply; (d) **the L3-precedent from J4 holds** — the pane is a consumer-wired dock pane (the standard `registerSidePanel` path, like Comments/Glossary), not new authority machinery.

The one substantiated adversarial finding, fixed: the pane first derived its live snapshot from `useStoreVersion` (which bumps on *selection* too) + `toSnapshot()`, so it re-ran the full per-proposal `applyProposal`+`diffSnapshots` on every **caret move** — a stepless commit that fires `subscribeCommit` and mints a fresh snapshot object but does NOT bump `revision` (`editor-store.ts:1351`). Fixed by deriving `current` from the shipped **commit-only** `useReviewSnapshot` and keying the per-proposal diff on `current.revision` (not the object), so navigation no longer re-diffs; plus wiring the store's registry into apply + diff so a custom object's `diffData` gives field-level detail. Proven: `engine-suggestion-source.test.ts` (registry idempotency/order/active; the router incl. no-op-collection filtering, settings field-count, conflict→settings→collection ordering; an end-to-end real-engine conflict + collection route) + `review-changes.spec.ts` (the pane lists a host proposal, routes the conflict + glossary change to the pane, resolves on accept) + legibility screenshots in `test-results/review-changes/`. `pnpm check` green.

### 5.8 J6 status — review-mode plumbing shipped (2026-07-02)

**J6's review-mode correctness layer is done.** It ships the live-store path J4/J5 intentionally avoided: `EditorStore.beginReviewMode`/`endReviewMode`, `applyProposalToStore`, `revertLiveProposalApplication`, `revertLiveProposalBlock`, the review-local `HistoryPool`, save gating, and the focused-block-protection handshake. Human edits during review record into the transient segment and get `origin:"suggested"`; programmatic optimistic apply/revert uses the same store dispatch chokepoint with `recordHistory:false` and `interactive:false`, so the main history stays frozen and the view does not reclaim focus for programmatic proposal movement. `CommittedTransaction.interactive` is the reclaim signal, replacing the old `origin === "local"` check in `react-view.tsx`; this closes the §13 bug where reviewer's suggested-origin structural edits could otherwise lose the caret. Public persistence now goes through `OwnedEditorHandle.canSave()` / `store.assertCanSaveSnapshot()`, and `useAutosave` waits while the review gate is closed instead of serializing unaccepted proposal content.

The focused-block handshake lives in the store because it needs parent-index and selection knowledge: before `applyProposalToStore` removes or reparents a focused subtree, `protectSelectionFromRemoval` moves the caret to a surviving text neighbour and reports `{from,to}` for UI notice. Reject runs in the safe order from §15: `revertReviewEdits()` replays the review segment's resident inverses, then `revertLiveProposalApplication()` applies the optimistic proposal inverse. Block-level resolution gets per-block inverse groups from the live apply result and cuts the review segment with `markReviewResolutionBoundary()`. Proven: `engine-review-mode.test.ts` covers review-local undo/redo, reject restoring the pre-review diff to zero, focused-block relocation, suggested-origin reviewer edits, and the public save gate. Story: `engine-review-mode.stories.tsx`.

### 5.9 J7 status — attribution wiring shipped (2026-07-02)

**J7's single-proposal attribution is done.** `proposalAttribution(proposal)` maps the active proposal author to a stable chip label and hue; `attributionForTextRun(proposal, run)` returns that author for identity-backed inserted/deleted `TextRunDiff` runs and `null` for unchanged or text-fallback runs. The active review surface accepts optional attribution and renders the author chip beside the change status, which matches §18's single-proposal rule: author is a session fact, not an in-prose color-disambiguation problem. Proven: `engine-suggestion-attribution.test.ts`; demonstrated in `engine-review-mode.stories.tsx`.
