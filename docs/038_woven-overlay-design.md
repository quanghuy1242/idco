...
Deferred, explicitly, with owners (the J-numbers match the `docs/036 §9` R6-J build order — the spike follow-ups below are **rendering productionization, which is J2**, not the canonical J1, which is the proposal model, §5.3):
- **J2 (spike follow-ups → ReviewModel productionization)** — content-aware ghost seed (route the base node into `seedFor`); incremental idle-coalesced re-diff (J0 runs a full `diffSnapshots` per commit); ghost height-cache eviction on review exit; the **hard no-tear gate** (a named test covering mobile EditContext-host flicker, cross-block Backspace/merge, and an edit that splices a ghost *newly adjacent* to the caret) plus a list-under-review regression case; and a free measurement nicety — `GhostBlock` uses `margin` (excluded from `borderBoxSize`, so ~8px/ghost stays missing from the treap *aggregate* total — cosmetic scrollbar drift with many ghosts, the same class as every margined block), swap to `padding` so the ghost's box is fully measured.
- **J2 (in-container ghosts)** — removed-child recursion + faithful container ghosts. Architecture note: the J0 ghost branch lives in the react-view top-level render loop; J2's in-container ghosts must splice into `block-dispatch.tsx`'s structural child-assembly, so before J2 relocate the ghost dispatch into `EngineBlock`/`block-dispatch` behind a review context rather than growing a second copy.
- **L** — `buildReviewOrder` does not yet pair `replaces`/`replacedBy` (a replacement renders as stacked ghost-then-added, not one unit).
