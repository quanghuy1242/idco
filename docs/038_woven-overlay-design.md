...
- **J1** — content-aware ghost seed (route the base node into `seedFor`); incremental idle-coalesced re-diff (J0 runs a full `diffSnapshots` per commit); ghost height-cache eviction on review exit; the **hard no-tear gate** (a named test covering mobile EditContext-host flicker, cross-block Backspace/merge, and an edit that splices a ghost *newly adjacent* to the caret) plus a list-under-review regression case; and a free measurement nicety — `GhostBlock` uses `margin` (excluded from `borderBoxSize`, so ~8px/ghost stays missing from the treap *aggregate* total — cosmetic scrollbar drift with many ghosts, the same class as every margined block), swap to `padding` so the ghost's box is fully measured.
- **J2** — removed-child recursion + faithful container ghosts. Architecture note: the J0 ghost branch lives in the react-view top-level render loop; J2's in-container ghosts must splice into `block-dispatch.tsx`'s structural child-assembly, so before J2 relocate the ghost dispatch into `EngineBlock`/`block-dispatch` behind a review context rather than growing a second copy.
