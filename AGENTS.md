...
## Public API surface docs (the consumer map)

Every package ships a generated API map at `packages/<pkg>/api/` (built by `scripts/gen-api-map.mjs` from the emitted `.d.ts`, regenerated by `pnpm build`, gitignored, included in the tarball). Consumer repos read that map instead of grepping idco source; the global `idco-consumer` skill drives them to it. The doc-comment standard that feeds it is `docs/032_api_surface_map_and_doc_standard.md`: every public (barrel-exported) symbol needs a one-sentence first-line summary and exactly one `@category` (set per file with `@categoryDefault`). `pnpm check:docs` is the gate — it fails if any public export is missing a summary or category, so a new export must be documented before it lands. Never hand-edit a file under `api/`.
