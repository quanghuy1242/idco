...
- W2. table model half → `core/table/` (C7). Rename `core/table.ts` to `core/table/definitions.ts`, move `view/table-operations.ts` to `core/table/operations.ts`, re-export both from `core/index.ts`. Fix importers: definitions in `structural-registry.ts`, `payload-import.ts`, `nodes/index.ts`; operations in `table-controls.tsx`, `table-interactions.tsx`, `text-block.tsx`, `tests/editor/engine-table.test.ts`. Done when: no `view/table-operations.ts` remains, behavior unchanged, `pnpm check` green.
...
1. Phase 0, relocation (W2). Create `core/table/`, move definitions + operations, fix imports. No behavior change; pre-positions every later table edit. Start here.
...
First action: Phase 0. Create `core/table/`, move `definitions.ts` and `operations.ts`, fix imports, and confirm `pnpm check` is green before any semantic edit.
